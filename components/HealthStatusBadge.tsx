...
const riskConfig: Record<
  RiskLevel,
  { color: string; bg: string; label: string; icon: string; message: string }
> = {
  low: {
    color: '#34C759',
    bg: '#34C75915',
    ...
    message: 'You are safe. Continue practicing good hygiene.',
  },
  moderate: {
    color: '#FF9500',
    bg: '#FF950015',
    label: 'Moderate Risk',
    icon: 'warning',
    message: 'Take precautions. Boil drinking water.',
  },
  high: {
    color: '#FF6B35',
    bg: '#FF6B3515',
    label: 'High Risk',
    icon: 'warning',
    message: 'Visit a health center for a check-up.',
  },
  critical: {
    color: '#FF3B30',
    bg: '#FF3B3015',
...
const HealthStatusBadge: React.FC<HealthStatusBadgeProps> = ({
  ...
  if (compact) {
    return (
      <View
        style={[styles.compactBadge, { backgroundColor: config.bg, borderColor: config.color }]}>
  ...
  return (
    <Animated.View
      ...
      {lastAssessed && <Text style={styles.timestamp}>Last assessed: {lastAssessed}</Text>}
    </Animated.View>
  );
};
