...
const LabCardItem = React.memo(({ lab, colors, styles, typeConfig, onCall, onDirections }: any) => {
    ...
    <View style={styles.labCard}>
      ...
      <View style={styles.actionsRow}>
        ...
        <TouchableOpacity
          style={styles.actionButton}
          activeOpacity={0.7}
          onPress={() => onDirections(lab.name, lab.address)}>
          <Text style={styles.actionText}>
...
const TestingLabs: React.FC = () => {
  const { data: labs, loading } = useSyncData('testingLabsData');
  ...
  const sortedLabs = useMemo(() => {
    if (!labs) return [];
    const filteredLabs = filter === 'all' ? labs : labs.filter((l: Lab) => l.type === filter);
    return [...filteredLabs].sort(
      (a: Lab, b: Lab) => parseFloat(a.distance) - parseFloat(b.distance)
    );
  }, [labs, filter]);
