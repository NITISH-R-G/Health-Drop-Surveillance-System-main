...
const LabItem = React.memo(
  ({
    ...
    onDirections: (name: string, address: string) => void;
  }) => {
    const cfg = typeConfig[lab.type as keyof typeof typeConfig] || typeConfig.both;
    ...
    const handleDirections = useCallback(() => {
      onDirections(lab.name, lab.address);
    }, [onDirections, lab.name, lab.address]);
    ...
    return (
      <View style={styles.labCard}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity style={styles.actionButton} activeOpacity={0.7} onPress={handleCall}>
...
const TestingLabs: React.FC = () => {
  ...
  const { data: labsData, loading } = useSyncData('testingLabs');

  const sortedLabs = useMemo(() => {
    if (!labsData) return [];
    const filteredLabs =
      filter === 'all' ? labsData : labsData.filter((l: Lab) => l.type === filter);
  ...
  const renderHeader = useCallback(
    ...
    [styles, colors, userLoc, isLocating, filter, handleGetLocation, setFilter]
  );
  ...
  const renderItem = useCallback(
    ...
    [styles, colors, handleCall, handleDirections]
  );
...
const createStyles = (colors: Theme) =>
  StyleSheet.create({
    ...
    filterRowContainer: { marginBottom: spacing.lg },
    filterRow: {},
    filterChip: {
      flexDirection: 'row',
      alignItems: 'center',
