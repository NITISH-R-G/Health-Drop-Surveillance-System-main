...
const stylesCache = new Map<Theme, ReturnType<typeof createStyles>>();
const getStyles = (colors: Theme) => {
  let styles = stylesCache.get(colors);
  if (!styles) {
    styles = createStyles(colors);
    stylesCache.set(colors, styles);
  }
  return styles;
};
...
const handleDirections = (
  name: string,
  address: string,
  userLoc: Location.LocationObject | null
) => {
...
const LabItem = React.memo(
  ({
    ...
    const onCall = useCallback(() => handleCall(lab.phone), [lab.phone]);
    const onDirections = useCallback(
      () => handleDirections(lab.name, lab.address, userLoc),
      [lab.name, lab.address, userLoc]
    );
    ...
    return (
      <View style={styles.labCard}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity style={styles.actionButton} activeOpacity={0.7} onPress={onCall}>
            <Text style={styles.actionText}>
              <Ionicons name="call" size={12} color={colors.primary} /> Call
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionButton} activeOpacity={0.7} onPress={onDirections}>
            <Text style={styles.actionText}>
...
const TestingLabs: React.FC<TestingLabsProps> = () => {
  const { colors } = useTheme();
  const styles = getStyles(colors);
  const { data: syncLabs } = useSyncData('testingLabsData');
  const labs = syncLabs || [];
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Lab List */}
      {sortedLabs.map((lab) => (
        <LabItem key={lab.id} lab={lab} styles={styles} colors={colors} userLoc={userLoc} />
      ))}
