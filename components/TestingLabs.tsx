...
const TestingLabs: React.FC<TestingLabsProps> = ({ labs }) => {
  ...
  const effectiveLabs = labs || syncedLabs || [];
  ...
  const sortedLabs = useMemo(() => {
    const filteredLabs =
      filter === 'all' ? effectiveLabs : effectiveLabs.filter((l) => l.type === filter);
    return [...filteredLabs].sort((a, b) => parseFloat(a.distance) - parseFloat(b.distance));
  }, [effectiveLabs, filter]);
  ...
  if (loading && (!effectiveLabs || effectiveLabs.length === 0)) {
    return (
