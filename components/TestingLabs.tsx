...
const TestingLabs: React.FC<TestingLabsProps> = ({ labs }) => {
  ...
  const { data: syncLabs, loading } = useSyncData('testingLabs');

  const activeLabs = labs || syncLabs || [];

  const sortedLabs = useMemo(() => {
    const filteredLabs = filter === 'all' ? activeLabs : activeLabs.filter((l: Lab) => l.type === filter);
