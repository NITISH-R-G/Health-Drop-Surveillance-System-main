...
const TestingLabs: React.FC<TestingLabsProps> = ({ labs }) => {
  ...
  const activeLabs = labs || syncedLabs || [];

  const sortedLabs = useMemo(() => {
    const filteredLabs = filter === 'all' ? activeLabs : activeLabs.filter((l) => l.type === filter);
  ...
  if (loading && !labs) {
    return (
      <View style={[styles.container, { padding: spacing.xl, alignItems: 'center' }]}>
        ...
        <Text style={{ ...typography.callout, color: colors.textSecondary, marginTop: spacing.md }}>
          Loading labs...
        </Text>
      </View>
    );
