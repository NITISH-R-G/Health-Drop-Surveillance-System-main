...
const LabCard = React.memo(({ lab, onCall, onDirections, colors, styles }: LabCardProps) => {
...
const TestingLabs: React.FC<TestingLabsProps> = () => {
  ...
  const { data: labs, loading } = useSyncData('testingLabs');

  const sortedLabs = useMemo(() => {
    const activeLabs = labs || [];
    const filteredLabs =
      filter === 'all' ? activeLabs : activeLabs.filter((l) => l.type === filter);
