...
const handleDirections = (name: string, address: string, userLoc: Location.LocationObject | null) => {
...
const LabItem = React.memo(({ lab, styles, colors, userLoc }: LabItemProps) => {
  const cfg = typeConfig[lab.type];

  const onCallPress = useCallback(() => handleCall(lab.phone), [lab.phone]);
  const onDirectionsPress = useCallback(() => handleDirections(lab.name, lab.address, userLoc), [lab.name, lab.address, userLoc]);
...
const TestingLabs: React.FC<TestingLabsProps> = ({ labs = defaultLabs }) => {
  ...
  ), [colors, filter, isLocating, styles, userLoc, handleGetLocation]);
  ...
  const renderItem = useCallback(({ item }: { item: Lab }) => (
    <LabItem lab={item} styles={styles} colors={colors} userLoc={userLoc} />
  ), [styles, colors, userLoc]);

  const keyExtractor = useCallback((item: Lab) => item.id, []);
