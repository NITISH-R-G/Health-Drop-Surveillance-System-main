...
const getRiskResult = (
  score: number
): { level: RiskLevel; color: string; title: string; advice: string } => {
  if (score <= 3)
    return {
      level: 'low',
      color: '#34C759',
      title: 'Low Risk',
      advice:
        'Your risk of water-borne illness is low. Continue using safe water sources and maintain hygiene practices.',
    };
  if (score <= 7)
    return {
      level: 'moderate',
      ...
      title: 'Moderate Risk',
      advice:
        'You have some risk factors. Ensure you boil or treat your drinking water and wash hands frequently. Monitor for symptoms.',
    };
  if (score <= 12)
    return {
      level: 'high',
      ...
      title: 'High Risk',
      advice:
        'Your risk is elevated. Please visit a health center for a check-up. Use only treated water and ORS if experiencing diarrhea.',
    };
  return {
    level: 'critical',
    color: '#FF3B30',
    title: 'Critical Risk',
    advice:
      'Seek medical attention immediately. You may have a water-borne infection. Visit the nearest hospital or call the helpline.',
  };
};
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={currentStep === 0 ? onClose : handleBack} activeOpacity={0.6}>
          <Ionicons
            name={currentStep === 0 ? 'close' : 'chevron-back'}
            size={24}
